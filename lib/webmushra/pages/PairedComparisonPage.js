...
function PairedComparisonPage(_reference, _condition, _pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _errorHandler, _language, _remoteService) {
...
PairedComparisonPage.prototype.render = function (_parent) {
  ...
  // On each load of the evaluation page, call a lambda that connects to the DB to wake it up
  div.append(`<script>fetch('${this.remoteService}/ping')</script>`)
  ...
  var trLoop = $("<tr style='display: flex; flex-direction: column; align-items: center;' id='trWs'></tr>");
  table.append(trLoop);
  ...
  var trNames = $("<tr><td>A</td><td>B</td><td></td></tr>");
